...
@router.delete("/courses/{course_id}/modules/{module_id}", response_model=pm.DefaultMessage, tags=["modules"])
def delete_module(course_id: int, module_id: int,
                  user: dict = UserDependency,
                  db_session = Depends(DBFacade().db_session)):
    '''
    Delete a module.
    ...
    # check if user is an admin
    if "admin" not in user["roles"] and "instructor" not in user["roles"]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only admins and instructors can delete modules")
    # check if course exists
    ...
    # check if course is created by the user
    if course.created_by != user["user_id"] and "admin" not in user["roles"]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only admins and course creators can delete modules")
    ...
    try:
        ...
        db_session.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
    return pm.DefaultMessage(message="Module deleted")